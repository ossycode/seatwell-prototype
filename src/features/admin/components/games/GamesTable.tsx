...
import { useDeleteGameMutation, useGetGamesQuery } from "@/services/gamesApi";
...
const GamesTable = () => {
  ...
  const createModalControl = useModal();
      ...
      <div className="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-white/[0.05] dark:bg-white/[0.03]">
        <div className="flex flex-col gap-2 px-5 mb-4 sm:flex-row sm:items-center sm:justify-between sm:px-6 ">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="flex flex-col sm:flex-row items-center gap-6">
              ...
              <div className="relative w-full">
                <Switch
                  label="Show Future Games Only"
                  defaultChecked={futureOnly}
          ...
          <div className="sm:px-6 lg:px-8 flex items-center justify-center mt-4 sm:mt-0 sm:justify-end gap-6">
            <button
              onClick={createModalControl.openModal}
              type="button"
                                ...
                                {new Date(game.date) < new Date() ? (
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <button
                                        disabled
                                        className="text-xs flex w-full rounded-lg px-3 py-2 text-left font-medium text-gray-400 dark:text-gray-500 cursor-not-allowed"
                                      >
                                        Edit
                                      </button>
                                    </TooltipTrigger>
                                    <TooltipContent>
                                      <p>Cannot edit past games</p>
                                    </TooltipContent>
                                  </Tooltip>
                                ) : (
                                  <button
        ...
        <CreateGameModal
          ...
          closeModal={createModalControl.closeModal}
          teams={teams ?? []}
        />
        {gameToEdit && (
          <EditGameModal
        ...
        {gameToDelete && (
          <WarningModal
            ...
            title="Delete Game"
            message={`Are you sure you want to delete this game for ${gameToDelete?.home_team?.name} vs
             ${gameToDelete.away_team?.name}? This action is irreversible.`}
